(()=>{
  const DB=[
    ...
    {keys:['oliwa z oliwek','oliwa','olej'],kcal:884,p:0,f:100,c:0,fi:0,su:0,salt:0},
    ...
    {keys:['mąka pszenna typ 00','maka pszenna typ 00','mąka pszenna','maka pszenna','mąka','maka'],kcal:364,p:10.3,f:1,c:76.3,fi:2.7,su:.3,salt:.01},
    {keys:['drożdże suche','drozdze suche'],kcal:325,p:40,f:7.6,c:41,fi:26.9,su:0,salt:.13},
    {keys:['drożdże świeże','drozdze swieze','drożdże','drozdze'],kcal:105,p:8.4,f:1.9,c:18,fi:8,su:0,salt:.08},
    {keys:['sól','sol'],kcal:0,p:0,f:0,c:0,fi:0,su:0,salt:100},
    {keys:['woda'],kcal:0,p:0,f:0,c:0,fi:0,su:0,salt:0},
  ...
  function grams(amount,unit,name){let a=Number(amount);if(!Number.isFinite(a)||a<=0)return 0;let u=norm(unit);if(UNITS[u]!=null)return a*UNITS[u];if(/jaj/.test(norm(name))&&!u)return a*60;return 0}
  ...
  function fillMacro(force=false){const r=estimateFromForm();if(!r||!r.matched)return false;const t=r.totals;const set=(id,v,d=1)=>{const e=document.getElementById(id);if(e&&(force||!Number(e.value)))e.value=Number(v.toFixed(d))};set('kcal',t.kcal,0);set('protein',t.p);set('fat',t.f);set('carbs',t.c);set('fiber',t.fi);set('sugars',t.su);set('salt',t.salt,2);let note=document.getElementById('notes');if(note&&r.confidence<1){const msg=`Makro oszacowane automatycznie z ${r.matched}/${r.total} rozpoznanych składników.`;if(!note.value.includes(msg))note.value=(note.value?note.value+'\n\n':'')+msg}return true}
  window.CookClipMacroEstimator={estimateFromForm,fillMacro};
  document.addEventListener('input',e=>{if(e.target.closest?.('#ingredients'))setTimeout(()=>fillMacro(true),120)});
  window.addEventListener('cookclip:imported',()=>setTimeout(()=>fillMacro(true),150));
})();