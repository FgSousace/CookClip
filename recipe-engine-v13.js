...
const FOOD=[
 ...
 {k:['drożdże suche','drozdze suche'],n:[325,40,7.6,41,26.9,0,.13]}, {k:['drożdże','drozdze'],n:[105,8.4,1.9,18,8,0,.08]},
 {k:['oliwa z oliwek','oliwa','olej'],n:[884,0,100,0,0,0,0]}, {k:['sól','sol'],n:[0,0,0,0,0,0,100]}, {k:['woda'],n:[0,0,0,0,0,0,0]},
 {k:['passata'],n:[30,1.4,.2,5.6,1.5,4,.3]}, {k:['mozzarella'],n:[280,28,17,3.1,0,1,1.6]}, {k:['parmezan'],n:[431,38,29,4.1,0,.9,3.8]}, {k:['ser'],n:[350,25,27,2,0,.5,1.8]},
 {k:['pierś z kurczaka','piers z kurczaka','kurczak'],n:[165,31,3.6,0,0,0,.18]}, {k:['ryż','ryz'],n:[360,7,.7,79,1.3,.1,.01]}, {k:['makaron'],n:[350,12,1.5,72,3,2,.02]},
 {k:['jajko','jajka'],n:[143,12.6,9.5,.7,0,.4,.36]}, {k:['mleko'],n:[61,3.2,3.3,4.8,0,4.8,.1]}, {k:['masło','maslo'],n:[717,.9,81,.1,0,.1,.03]},
 {k:['pomidor','pomidory'],n:[18,.9,.2,3.9,1.2,2.6,.01]}, {k:['cebula'],n:[40,1.1,.1,9.3,1.7,4.2,.01]}, {k:['czosnek'],n:[149,6.4,.5,33,2.1,1,.04]},
 {k:['cukier'],n:[400,0,0,100,0,100,0]}, {k:['jogurt'],n:[63,5.3,1.6,7,0,7,.12]}, {k:['ziemniak','ziemniaki'],n:[77,2,.1,17,2.2,.8,.02]}, {k:['banan'],n:[89,1.1,.3,23,2.6,12,.01]}
];
...
function extractIngredients(text){
 ...
 const U='kg|mg|g|ml|cl|dl|l|szt\\.?|sztuki?|łyżki?|łyżka|łyżeczki?|łyżeczka|szklanki?|szklanka|ząbki?|ząbek|opak\\.?|opakowanie|garść|cup|cups|tbsp|tsp';
 const Q='\\d+\\/\\d+|\\d+(?:[.,]\\d+)?';
 const stop=`(?=\\s+(?:${Q})\\s*(?:${U})(?=\\s)|\\s+\\d+\\s*(?:h|godzin\\w*|minut\\w*)\\b|[,;|\\n]|$)`;
 const re=new RegExp(`(${Q})\\s*(${U})(?=\\s|$|[,])\\s*([^,;|\\n]{1,100}?)${stop}`,'gi'); let m;
...
function nval(v,d=1){return Number.isFinite(v)&&v>0?Number(v.toFixed(d)):''}
function apply(d,url){resetEditor();title.value=d.title;servings.value=d.servings;sourceUrl.value=url;sourceName.value=d.author?`TikTok — ${d.author}`:'TikTok';ingredients.innerHTML='';d.ingredients.forEach(ingRow);if(!d.ingredients.length)ingRow();steps.innerHTML='';d.steps.forEach(stepRow);if(!d.steps.length)stepRow();kcal.value=nval(d.macro.kcal,0);protein.value=nval(d.macro.p);fat.value=nval(d.macro.f);carbs.value=nval(d.macro.c);fiber.value=nval(d.macro.fi);sugars.value=nval(d.macro.su);salt.value=nval(d.macro.salt,2);notes.value=d.notes;importDialog.close();editorTitle.textContent=`Sprawdź przepis z TikToka • v${V}`;editor.showModal()}
btn.onclick=async function(ev){const target=input.value.trim();if(!isTikTok(target)){if(typeof fallback==='function')return fallback.call(this,ev);return}btn.disabled=true;try{status.textContent='🧠 CookClip v14: analizuję opis TikToka…';const full=await resolveTikTok(target),d=await oembed(full);if(!d)throw Error('TikTok nie zwrócił opisu');const caption=clean(d.title||'');const ingredients=extractIngredients(caption),steps=extractSteps(caption),sv=servingsFrom(caption),exp=explicitMacro(caption),est=calcMacro(ingredients,sv);const hasExplicit=exp.kcal||exp.p||exp.f||exp.c;const macro=hasExplicit?{...est,...exp}:{...est};const titleText=clean(d.title||'').replace(/#\S+/g,' ').slice(0,100)||'Przepis z TikToka';const notes=`Źródłowy opis TikToka:\n${caption}\n\n${ingredients.length?`Rozpoznano ${ingredients.length} wiarygodnych składników.`:'Nie znaleziono wiarygodnych składników z ilościami.'}${est.matched?` Makro oszacowano z ${est.matched} składników.`:''}`;apply({title:titleText,author:d.author_name||'',ingredients,steps,servings:sv,macro,notes},full);status.textContent=`✅ v14: ${ingredients.length} składników, ${steps.length} kroków${est.matched?', makro policzone':''}.`;}catch(e){status.textContent='❌ v14: '+(e.message||e)}finally{btn.disabled=false}};
})();