const CACHE='cookclip-v10';
const ASSETS=['./manifest.webmanifest','./import-fix.js','./tiktok-smart-import.js','./macro-estimator.js','./macro-import-hook.js'];
...
self.addEventListener('fetch',e=>{
  ...
  if(url.origin===self.location.origin && (url.pathname.endsWith('/CookClip/')||url.pathname.endsWith('/CookClip/index.html'))){
    e.respondWith(fetch(e.request,{cache:'no-store'}).then(async r=>{
      ...
      html=html.replace('</body>',`<script src="./import-fix.js?v=10"></script><script src="./tiktok-smart-import.js?v=10"></script><script src="./macro-estimator.js?v=10"></script><script src="./macro-import-hook.js?v=10"></script></body>`);
